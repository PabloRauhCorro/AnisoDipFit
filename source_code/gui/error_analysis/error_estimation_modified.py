...
class ErrorEstimation():
    def calculate_score_vs_parameters(self, best_parameters, err_settings, fit_settings, simulator, exp_data, spinA,
                                      spinB, calc_settings, error_analysis_thread):
        ...
        for i in range(Ne):
            ...
            for name in const['variable_names']:
                ...
                if not (index == -1) and not (name in err_settings['variables'][i]):
                    for j in range(Ns):
                        self.generation.chromosomes[j].genes[index] = best_parameters[name]['value']
            # Score the generation
            print("\nScoring the generation\n")
            ...
            for name in err_settings['variables'][i]:
                ...
                index = fit_settings['parameters']['indices'][name]
                for j in range(Ns):
                    score_vs_variables[name].append(self.generation.chromosomes[j].genes[index])
            score_vs_variables['score'] = []
            for j in range(Ns):
                score_vs_variables['score'].append(self.generation.chromosomes[j].score)
            score_vs_parameters.append(score_vs_variables)
            percentage_value = 100 * (i+1)/Ne
            self.GUI_progressbar_updater(error_analysis_thread, int(percentage_value) )
        sys.stdout.write('\n')
        print('Done with score vs parameters \n')
        return score_vs_parameters

    def calculate_parameter_errors(self, score_vs_parameters, best_parameters, err_settings, fit_settings, simulator,
                                   exp_data, spinA, spinB, calc_settings, error_analysis_thread):
        ...
        numerical_error = self.calculate_numerical_error(best_parameters, err_settings, fit_settings, simulator,
                                                         exp_data, spinA, spinB, calc_settings)
        self.GUI_progressbar_updater(error_analysis_thread, 95)
        sys.stdout.write("Numerical error (chi2) = %f\n" % (numerical_error))
        # Set the score threshold
        score_threshold = self.calculate_score_threshold(err_settings['confidence_interval'], numerical_error, 1)
        ...
        for i in range(Ne):
            for name in err_settings['variables'][i]:
                ...
                parameter_error = self.calculate_parameter_error(parameter_values, score_values, score_threshold)
                if name in parameter_errors:
                    ...
                    parameter_errors[name] = parameter_error
        self.GUI_progressbar_updater(error_analysis_thread, 100)
        return [parameter_errors, numerical_error]

    def calculate_numerical_error(self, best_parameters, err_settings, fit_settings, simulator, exp_data, spinA, spinB,
                                  calc_settings):
        # Create a generation
        Ns = err_settings['Ns']
        self.generation = Generation(Ns)
        ...
        score_min = self.generation.chromosomes[0].score
        score_max = self.generation.chromosomes[Ns - 1].score
        numerical_error = score_max - score_min
        return numerical_error

    def calculate_score_threshold(self, confidence_interval, numerical_error, degree_of_freedom):
        score_threshold = 0.0
    ...
    def calculate_parameter_error(self, parameter_values, score_values, score_threshold):
