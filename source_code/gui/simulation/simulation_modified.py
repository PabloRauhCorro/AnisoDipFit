...
class Simulator:
    ...
    def dipolar_spectrum_vs_xi(self, parameters, spinA, spinB, simulation_thread = False):
        Nf = self.Nf
        ...
        for i in range(Nxi):
            ...
            sys.stdout.flush()
            self.GUI_progressbar_updater(simulation_thread, int(status))
    ...
    def dipolar_spectrum_vs_phi(self, parameters, spinA, spinB, simulation_thread = False):
        Nf = self.Nf
    ...
    def dipolar_spectrum_vs_temp(self, fdd, weights_xi, spinA, spinB, calc_settings, simulation_thread = False):
        Nf = self.Nf
    ...
    def run_simulation(self, sim_settings, exp_data, spinA, spinB, calc_settings, simulation_thread = False):
        sys.stdout.write('Starting the simulation...\n')
        self.GUI_statusbar_updater(simulation_thread, 'Starting the simulation...')
        sys.stdout.write('Running pre-calculations... ')
        self.GUI_statusbar_updater(simulation_thread, 'Running the pre-calculations... ')
        # Set the frequency axis
        ...
        if (calc_settings['g_selectivity'] and spinB['type'] == "anisotropic"):
            self.weights_temp = flip_probabilities(self.gB, spinB['g'], calc_settings['magnetic_field'],
        ...
        if (sim_settings['modes']['spc'] or
                ...
                sim_settings['modes']['spc_vs_theta'] or
                sim_settings['modes']['spc_vs_temp']):
            fdd, theta, weights_xi = self.dipolar_frequencies(sim_settings['parameters'], spinA, spinB, self.gA,
                                                              self.gB, self.qA, self.qB,
                                                              sim_settings['modes']['spc_vs_theta'])
        ...
        if sim_settings['modes']['timetrace']:
            ...
            self.GUI_progressbar_updater(simulation_thread, 0)
            self.GUI_statusbar_updater(simulation_thread, 'Calculating the dipolar time trace... ')
        ...
        if (sim_settings['modes']['spc_vs_theta']):
            ...
            self.GUI_statusbar_updater(simulation_thread, 'Calculating the dipolar spectrum vs theta... ')
        ...
        if sim_settings['modes']['spc_vs_xi']:
            ...
            self.GUI_statusbar_updater(simulation_thread, 'Calculating the dipolar spectrum vs xi...             ')
            self.xi_bins = sim_settings['settings']['xi_bins']
            self.spc_vs_xi = self.dipolar_spectrum_vs_xi(sim_settings['parameters'], spinA, spinB, simulation_thread)
            sys.stdout.write('[DONE]\n')
        ...
        if sim_settings['modes']['spc_vs_phi']:
            ...
            self.GUI_statusbar_updater(simulation_thread, 'Calculating the dipolar spectrum vs phi...             ')
            self.phi_bins = sim_settings['settings']['phi_bins']
            self.spc_vs_phi = self.dipolar_spectrum_vs_phi(sim_settings['parameters'], spinA, spinB, simulation_thread)
            sys.stdout.write('[DONE]\n')
        ...
        if (sim_settings['modes']['spc_vs_temp'] and calc_settings['g_selectivity']):
            ...
            self.GUI_statusbar_updater(simulation_thread, 'Calculating the dipolar spectrum vs temperature...         ')
            self.temp_bins = sim_settings['settings']['temp_bins']
            self.spc_vs_temp, self.g, self.depth_vs_temp = self.dipolar_spectrum_vs_temp(fdd, weights_xi, spinA, spinB,
                                                                                         calc_settings, simulation_thread)
            sys.stdout.write('[DONE]\n')
        sys.stdout.write('The simulation is finished\n\n')
        self.GUI_statusbar_updater(simulation_thread, 'The simulation is finished')
    ...
    def init_fitting(self, fit_settings, exp_data, spinA, spinB, calc_settings):
        ...
        self.field, self.gA, self.gB, self.qA, self.qB = self.spin_ensemble(spinA, spinB)
        # Calculate the temperature-based weights
        if (fit_settings['parameters']['indices']['temp'] == -1 and calc_settings['g_selectivity'] and spinB[
            'type'] == "anisotropic"):
            self.weights_temp = flip_probabilities(self.gB, spinB['g'], calc_settings['magnetic_field'],
                                                   fit_settings['parameters']['fixed']['temp'])

    def GUI_statusbar_updater(self, simulation_thread, status_message):
        if simulation_thread:
            simulation_thread.status_message = status_message
            simulation_thread.statusbar_signal.emit(simulation_thread.status_message)

    def GUI_progressbar_updater(self, simulation_thread, progress_value):
        if simulation_thread:
            simulation_thread.progress_value = progress_value
            simulation_thread.progressbar_signal.emit(simulation_thread.progress_value)